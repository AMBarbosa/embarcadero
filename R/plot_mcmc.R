...
#' @param col Color palette for the maps
#'
#' @export

plot.mcmc <- function(object, inputstack, iter=100, wait=0.1, quiet=FALSE, col = rev(terrain.colors(255))) {
  ...
  # input.matrix <- input.matrix[complete.cases(input.matrix),]  # disabled so raster pixels don't get disorganized because of NA removal
  ...
  for (i in 1:iter){
    ...
    par(mfrow=c(1,2))
    par(mar=c(2,1,2,5))
    plot(r, col = col, box=F, axes=F, zlim=c(0,1), main=paste('Iter',i))
    plot(r1/i, col = col, box=F, axes=F, zlim=c(0,1), main='Mean')
    Sys.sleep(0.1)
